	...
	<?php
		...
		switch ($urlsrtby) {
			case '1':
				$srtby = "1";
				if($pcid=="all"){
					$sqlProducts = "SELECT * FROM store_product ORDER BY product_ID DESC";
				}

				else{
					$sqlProducts = "SELECT * FROM store_product p, store_categories c WHERE p.product_Category_ID = c.categort_ID AND product_Category_ID= '$pcid' ORDER BY p.product_ID DESC";
				}
				break;
			...
			case '2':
				$srtby = "2";
				if($pcid=="all"){
					$sqlProducts = "SELECT * FROM store_product ORDER BY product_ID ASC";
				}

				else{
					$sqlProducts = "SELECT * FROM store_product p, store_categories c WHERE p.product_Category_ID = c.categort_ID AND product_Category_ID= '$pcid' ORDER BY p.product_ID ASC";
				}
				break;
			...
			case '3':
				...
				if($pcid=="all"){
					...
					$sqlProducts = "SELECT * FROM store_product p, store_categories c WHERE p.product_Category_ID = c.categort_ID AND product_Category_ID= '$pcid' ORDER BY p.product_Price ASC";
				}
				break;
			...
			case '4':
				...
				if($pcid=="all"){
					...
					$sqlProducts = "SELECT * FROM store_product p, store_categories c WHERE p.product_Category_ID = c.categort_ID AND product_Category_ID= '$pcid' ORDER BY p.product_Price DESC";
				}
				break;	
		...
		$slqcategories2 = "SELECT * FROM store_categories WHERE categort_ID ='" .$pcid. "'" ;
			...
			<div class="col-12 col-sm-12 col-md-3">
				...
				<div class="sort_by">
					...
					<select class="sortbyitems" id="sortbyitems">
						...
						<option value="product.php?category=<?php echo $pcid ;?>"
							<?php if($srtby==0){
								echo "selected";
							} ?>
							>--Select--</option>

						<option value="product.php?category=<?php echo $pcid ;?>&sortby=1"
							<?php if($srtby==1){
								echo "selected";
							} ?>
							>Date(New to Oldest)</option>
						<option value="product.php?category=<?php echo $pcid ;?>&sortby=2"
							<?php if($srtby==2){
								echo "selected";
							} ?>
							>Date(Oldest to New)</option>
						<option style="display:none;" value="product.php?category=<?php echo $pcid ;?>&sortby=3"
							<?php if($srtby==3){
								echo "selected";
							} ?>
							>Price(Low to High)</option>
						<option style="display:none;"  value="product.php?category=<?php echo $pcid ;?>&sortby=4"
							<?php if($srtby==4){
								echo "selected";
							} ?>
				...
				?>
							<div class="col-12 col-sm-6 col-md-4">
								<div class="store_products" style="padding-bottom: 0.5em; margin-bottom: 0.5em; cursor: pointer;" onclick="window.location = 'item.php?productid=<?php echo $proID;?>'">
									...
									<div class="product_des" style="min-height: 6em;">
